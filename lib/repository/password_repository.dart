...
class PasswordRepository extends ChangeNotifier {
  ...
  Future<void> loadPasswords(String userId) async {
    ...
    try {
      ...
      _passwords =
          querySnapshot.docs.map((doc) => Password.fromFirestore(doc)).toList();
  ...
  Future<bool> addPassword(Password password) async {
    try {
      await _firestore.collection('cofre').add(password.toFirestore());
  ...
  Future<bool> moveToTrash(String passwordId, String userId) async {
    try {
      final passwordDoc =
          await _firestore.collection('cofre').doc(passwordId).get();
      ...
      await _firestore.collection('lixeira').doc(passwordId).set(trashData);

      await _firestore.collection('cofre').doc(passwordId).delete();
  ...
  Future<bool> deletePassword(String passwordId, String userId) async {
    try {
      await _firestore.collection('lixeira').doc(passwordId).delete();
  ...
  Future<bool> restoreFromTrash(String passwordId, String userId) async {
    try {
      final trashDoc =
          await _firestore.collection('lixeira').doc(passwordId).get();
      ...
      final restoredData =
          password.copyWith(isDeleted: false, deletedAt: null).toFirestore();

      restoredData.remove('isDeleted');
      restoredData.remove('deletedAt');
      restoredData['updatedAt'] = DateTime.now();

      await _firestore.collection('cofre').doc(passwordId).set(restoredData);
  ...
  List<Password> getSortedPasswords({
    ...
    switch (criteria) {
      ...
      case SortCriteria.site:
        sortedList.sort(
          (a, b) =>
              ascending ? a.site.compareTo(b.site) : b.site.compareTo(a.site),
        );
        break;
  ...
  Stream<List<Password>> watchPasswords(String userId) {
    return _firestore
        ...
        .map((snapshot) {
          final passwords =
              snapshot.docs.map((doc) => Password.fromFirestore(doc)).toList();
